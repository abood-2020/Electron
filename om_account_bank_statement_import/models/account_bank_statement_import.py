...
class AccountBankStatementLine(models.Model):
    ...
    unique_import_id = fields.Char(string='Import ID', readonly=True)
    ...
    def update_unique_id(self):
            for rec in self: 
                # base_qry = "update account_bank_statement_line set unique_import_id = %s ;"
                string= str(rec.date) + '#' + str(rec.payment_ref) + '#' + str(rec.ref)+ str(rec.name)
                # self.env.cr.execute(base_qry,[string])
                rec.unique_import_id = string
...
class AccountBankStatementImport(models.TransientModel):
    ...
    def update_statement(self, values):
        statement = self.env['account.bank.statement'].write(values)
        return statement

    def import_file(self):
        for data_file in self.attachment_ids:
            file_name = data_file.name.lower()
            if file_name.strip().endswith('.csv') or file_name.strip().endswith(('.xlsx','.xls')):
                statement = False
                if file_name.strip().endswith('.csv'):
                    ...
                    for i in range(len(file_reader)):
                        ...
                        if values:
                            if i == 0:
                                ...
                                if not date:
                                    date = field[1]
                                    # date_to_import = datetime.strptime(date, '%d-%m-%Y').strftime('%y-%m-%d')
                                # values from original module                               
                                # values.update({
                                #     'date': field[0],
                                #     'payment_ref': field[1],
                                #     'ref': field[2],
                                ...
                                # })
                                values.update({
                                    'unique_import_id': field[0],
                                    'date': datetime.strptime(field[1], '%d-%m-%Y').strftime('%Y-%m-%d'),
                                    'payment_ref': field[2],
                                    'ref': field[3],
                                    'partner_id': self.get_partner(field[4]),
                                    'amount': field[5],
                                    'currency_id':  self.get_currency(field[6])
                                })
                ...
                elif file_name.strip().endswith(('.xlsx','.xls')):
                    try:
                    ...
                    line_date = datetime.today().date()
                    for row_no in range(sheet.nrows):
                        val = {}
                        values = {}
                        import_end_balance = 0.0
                        if row_no <= 0:
                            ...
                            unique_import_id_str = line[0]+"#"+line[2]+"#"+line[6]+"#"+line[7]
                            statment_lines = self.env['account.bank.statement.line'].search([('journal_id','=',self.env.context.get('active_id')),('unique_import_id','=',unique_import_id_str)])
                            if line[2] == "":
                                amount_ = line[3].replace(',',"")
                                amount = amount_.replace(' ',"")
                            else:
                                amount_ = line[2].replace(',',"")
                                amount = amount_.replace(' ',"")
                            if statment_lines:
                                continue
                                # raise UserError(_("Did true "))
                            else:
                                line_date = datetime.strptime(line[0], '%d-%m-%Y').strftime('%Y-%m-%d')   
                                import_end_balance += float(amount)
                                values.update({
                                        'unique_import_id': unique_import_id_str,
                    ...
                    statement_vals = {
                        'name': 'Statement Of ' + str(line_date),
                        'journal_id': self.env.context.get('active_id'),
                        'line_ids': vals_list
                    }
                    monthly_statement = self.env['account.bank.statement'].search([('journal_id','=',self.env.context.get('active_id')),('name','=','Statement Of ' + str(line_date))])
                    if monthly_statement:
                        statement_vals = {
                            # 'id':monthly_statement.id,
                            'line_ids': vals_list,
                            'balance_end_real': monthly_statement.balance_end + import_end_balance
                        }
                        # raise UserError(_(vals_list))
                        statement_update = monthly_statement.write(statement_vals)
                    else:
                        if len(vals_list) != 0:
                            statement = self.create_statement(statement_vals)
                if statement:
                ...
                elif statement_update:
                    return {
                        'type': 'ir.actions.act_window',
                        'res_model': 'account.bank.statement',
                        'view_mode': 'form',
                        'res_id': monthly_statement.id,
                        'views': [(False, 'form')],
                    }
            else:
